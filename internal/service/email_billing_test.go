...
func TestEmailBillingInviteBonus(t *testing.T) {
	...
	if wallet["invitee_count"] != 1 {
		t.Fatalf("invitee_count = %#v, want 1", wallet["invitee_count"])
	}
	invitees, _ := wallet["invitees"].([]map[string]any)
	if len(invitees) != 1 || invitees[0]["email"] != "newuser@qq.com" {
		t.Fatalf("invitees = %#v", wallet["invitees"])
	}
}

func TestEmailBillingListOrdersIncludesRedeemAndAdminAdjust(t *testing.T) {
	dataDir := t.TempDir()
	backend := storage.NewJSONBackend(
		filepath.Join(dataDir, "accounts.json"),
		filepath.Join(dataDir, "auth_keys.json"),
	...
	_, key, err := billing.RegisterEmailUser("orders@qq.com", "password123", "orders", "", "", 8, []string{"qq.com"}, EmailSMTPConfig{})
	if err != nil {
		t.Fatalf("RegisterEmailUser() error = %v", err)
	}
	identity := auth.Authenticate(key)
	...
	if _, err := billing.AdminAdjustUserBalance(identity.ID, 50, "admin topup"); err != nil {
		t.Fatalf("AdminAdjustUserBalance() error = %v", err)
	}
	codes, err := billing.CreateRedeemCodes(120, 1, "", "promo")
	if err != nil || len(codes) != 1 {
		t.Fatalf("CreateRedeemCodes() err=%v codes=%#v", err, codes)
	}
	code, _ := codes[0]["code"].(string)
	if code == "" {
		t.Fatalf("created redeem code = %#v", codes[0])
	}
	if _, err := billing.RedeemCode(*identity, code); err != nil {
		t.Fatalf("RedeemCode() error = %v", err)
	}
	if _, err := billing.RedeemCode(*identity, "INVALID-CODE"); err == nil {
		t.Fatal("RedeemCode(invalid) should fail")
	}

	orders := billing.ListOrdersByIdentity(*identity, 30)
	if len(orders) < 3 {
		t.Fatalf("ListOrdersByIdentity() = %#v", orders)
	}
	foundAdmin := false
	foundRedeem := false
	foundRedeemFailed := false
	for _, item := range orders {
		if item["provider"] == BillingProviderAdmin && item["status"] == BillingOrderStatusPaid {
			foundAdmin = true
		}
		if item["provider"] == BillingProviderRedeemCode && item["status"] == BillingOrderStatusPaid {
			foundRedeem = true
		}
		if item["provider"] == BillingProviderRedeemCode && item["status"] == BillingOrderStatusFailed {
			foundRedeemFailed = true
		}
	}
	if !foundAdmin || !foundRedeem || !foundRedeemFailed {
		t.Fatalf("expected admin/redeem success+failed in orders, got %#v", orders)
	}
}
