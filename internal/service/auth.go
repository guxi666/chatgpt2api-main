...
func mergeManagedAuthUser(user, item map[string]any) {
	...
	if email := util.Clean(item["email"]); email != "" {
		user["email"] = email
	}
...
func mergeDefaultManagedRole(roles []ManagedRole) []ManagedRole {
	...
	for _, role := range roles {
		...
		if role.ID == DefaultManagedRoleID {
			...
			// Always keep built-in default role aligned with latest baseline permissions.
			// Preserve any manually-added permissions while ensuring baseline permissions exist.
			role.MenuPaths = NormalizeMenuPermissions(append(append([]string(nil), role.MenuPaths...), defaultRole.MenuPaths...))
			role.APIPermissions = NormalizeAPIPermissions(append(append([]string(nil), role.APIPermissions...), defaultRole.APIPermissions...))
			out = append(out, role)
			seenDefault = true
			continue
		}
