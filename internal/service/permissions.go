...
var fullMenuPermissions = []MenuPermission{
	...
	{ID: "subscription", Label: "套餐订阅", Path: "/subscription", Icon: "gem", Order: 26},
...
var apiPermissionCatalog = []APIPermission{
	...
	apiPermission("GET", "/api/images/fetch", "下载图片文件", "图片库", false),
	...
	apiPermission("GET", "/api/wallet", "查看钱包", "钱包充值", false),
	apiPermission("POST", "/api/wallet/redeem", "兑换卡密", "钱包充值", false),
	apiPermission("GET", "/api/pay/orders", "查看充值记录", "钱包充值", false),
	apiPermission("POST", "/api/pay/orders", "创建充值订单", "钱包充值", false),
	apiPermission("GET", "/api/subscriptions/plans", "查看套餐订阅", "套餐订阅", false),
	apiPermission("POST", "/api/subscriptions/orders", "创建套餐订单", "套餐订阅", false),
	apiPermission("GET", "/api/agency", "查看代理加盟价格", "代理加盟", false),
	apiPermission("POST", "/api/agency", "修改代理加盟价格", "代理加盟", false),
	apiPermission("GET", "/api/agency/commission", "查看代理分成", "代理加盟", false),
	apiPermission("GET", "/api/agency/withdrawals", "Agency withdrawals", "Agency", false),
	apiPermission("POST", "/api/agency/withdrawals", "Create agency withdrawal", "Agency", false),
	...
	apiPermission("POST", "/api/agency/admin/users", "后台开通代理", "代理加盟", false),
	...
	apiPermission("GET", "/api/admin/billing", "查看计费管理", "计费管理", true),
	apiPermission("POST", "/api/admin/billing", "修改计费配置与余额", "计费管理", true),
	apiPermission("DELETE", "/api/admin/billing", "删除计费资源", "计费管理", true),
...
func DefaultPermissionSetForRole(role string) PermissionSet {
	...
	return PermissionSet{
		...
		APIPermissions: NormalizeAPIPermissions([]string{
			...
			APIPermissionKey("GET", "/api/wallet"),
			APIPermissionKey("POST", "/api/wallet/redeem"),
			APIPermissionKey("GET", "/api/pay/orders"),
			APIPermissionKey("POST", "/api/pay/orders"),
			APIPermissionKey("GET", "/api/subscriptions/plans"),
			APIPermissionKey("POST", "/api/subscriptions/orders"),
			APIPermissionKey("GET", "/api/agency"),
			APIPermissionKey("POST", "/api/agency/join"),
			APIPermissionKey("POST", "/api/agency/upgrade"),
		}),
	}
}
