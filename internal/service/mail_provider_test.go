...
func TestRegisterCloudflareTempMailProviderRandomSubdomain(t *testing.T) {
	testCases := []struct {
		name            string
		domain          []string
	...
	}{
		{
			...
			pattern:         regexp.MustCompile(`^[a-z0-9]{4,10}\.example\.test$`),
		},
		{
			name:            "root domain with random_subdomain",
			domain:          []string{"example.test"},
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				w.Header().Set("Content-Type", "application/json")
				_, _ = w.Write([]byte(`{"address":"user@example.test","jwt":"jwt-1"}`))
			}))
			defer server.Close()
			...
			provider, err := createRegisterMailProvider(map[string]any{
				...
				"providers": []map[string]any{{
					...
					"domain":           tc.domain,
					"random_subdomain": tc.randomSubdomain,
				}},
			}, "", "")
			if err != nil {
				t.Fatalf("createRegisterMailProvider() error = %v", err)
			...
			if mailbox["provider"] != "cloudflare_temp_email" || mailbox["address"] != "user@example.test" || mailbox["token"] != "jwt-1" {
				t.Fatalf("mailbox = %#v", mailbox)
			}
			if generatedPayload["enablePrefix"] != true || generatedPayload["name"] != "user" {
				t.Fatalf("generated payload = %#v", generatedPayload)
			}
			domain, _ := generatedPayload["domain"].(string)
			if !tc.pattern.MatchString(domain) {
				t.Fatalf("payload domain = %q, want pattern %q", domain, tc.pattern.String())
			}
		})
	}
}
