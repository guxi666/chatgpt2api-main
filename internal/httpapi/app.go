...
func NewApp() (*App, error) {
	...
	billing := service.NewEmailBillingService(cfg.DataDir, storageBackend, auth)
...
func (a *App) serveHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case path == "/auth/register" && r.Method == http.MethodPost:
		a.handleEmailRegister(w, r)
	case path == "/auth/email-register-code" && r.Method == http.MethodPost:
		a.handleEmailRegisterCode(w, r)
	case path == "/auth/email-login" && r.Method == http.MethodPost:
		a.handleEmailLogin(w, r)
	...
	case path == "/api/wallet":
		a.handleWallet(w, r)
	case path == "/api/wallet/redeem":
		a.handleWalletRedeem(w, r)
	case path == "/api/pay/orders":
		a.handlePayOrders(w, r)
	case path == "/api/pay/yipay/notify":
		a.handleYiPayNotify(w, r)
	case path == "/api/admin/billing/users" || strings.HasPrefix(path, "/api/admin/billing/users/") || path == "/api/admin/billing/redeem-codes" || strings.HasPrefix(path, "/api/admin/billing/redeem-codes/"):
		a.handleAdminBilling(w, r)
...
func (a *App) handleChatCompletions(w http.ResponseWriter, r *http.Request) {
	...
	if isChargeableChatCompletionsRequest(body) {
		if !a.ensureImageBillingCredit(w, identity) {
			return
		}
	} else {
		if !a.ensureChatBillingCredit(w, identity) {
			return
		}
	}
	body["owner_id"] = identityScope(identity)
...
func (a *App) handleResponses(w http.ResponseWriter, r *http.Request) {
	...
	if isChargeableResponsesRequest(body) && !a.ensureImageBillingCredit(w, identity) {
		return
	}
	body["owner_id"] = identityScope(identity)
	model := firstNonEmpty(util.Clean(body["model"]), "auto")
...
func (a *App) writeProtocol(w http.ResponseWriter, r *http.Request, result map[string]any, stream *protocol.StreamResult, err error, sseKind, endpoint, model string, identity service.Identity, summary string) {
	...
	if stream == nil {
		if isChargeableImageEndpoint(endpoint) && hasImageResult(result) {
			if chargeErr := a.chargeImageUsage(identity, endpoint); chargeErr != nil {
				a.logCall(identity, summary, endpoint, model, start, "failed", chargeErr.Error(), nil)
	...
	if stream.Kind == "anthropic" || sseKind == "anthropic" {
		...
		if isChargeableImageEndpoint(endpoint) && len(urls) > 0 {
			if chargeErr := a.chargeImageUsage(identity, endpoint); chargeErr != nil {
				a.recordImageOwners(identity, urls)
				a.logCall(identity, summary, endpoint, model, start, "failed", chargeErr.Error(), urls)
				fmt.Fprintf(w, "event: error\n")
				fmt.Fprintf(w, "data: %s\n\n", jsonString(map[string]any{"type": "error", "error": map[string]any{"type": "billing_error", "message": chargeErr.Error()}}))
				return
			}
		}
		a.recordImageOwners(identity, urls)
	...
	if err := <-stream.Err; err != nil {
		...
		if isChargeableImageEndpoint(endpoint) && len(urls) > 0 {
			if chargeErr := a.chargeImageUsage(identity, endpoint); chargeErr != nil {
				a.recordImageOwners(identity, urls)
				a.logCall(identity, summary, endpoint, model, start, "failed", chargeErr.Error(), urls)
				fmt.Fprintf(w, "data: %s\n\n", jsonString(openAIErrorForStream(chargeErr)))
...
func (a *App) handleAppMeta(w http.ResponseWriter, r *http.Request) {
	util.WriteJSON(w, http.StatusOK, map[string]any{
		...
		"app_logo_url":                a.config.AppLogoURL(),
...
func (a *App) runLoggedImageTask(ctx context.Context, identity service.Identity, payload map[string]any, endpoint, summary string, run func(context.Context, map[string]any) (map[string]any, error)) (map[string]any, error) {
	...
	if isChargeableImageEndpoint(endpoint) {
		if chargeErr := a.chargeImageUsage(identity, endpoint); chargeErr != nil {
			a.logCall(identity, summary, endpoint, model, start, "failed", chargeErr.Error(), urls)
			return map[string]any{"data": []map[string]any{}, "message": chargeErr.Error()}, chargeErr
		}
	}
	a.logCall(identity, summary, endpoint, model, start, "success", "", urls)
	return result, nil
