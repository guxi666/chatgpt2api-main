...
func TestStoreUpdatePersistsEmailSMTPWithoutLeakingAuthCode(t *testing.T) {
	root := t.TempDir()
	t.Setenv("CHATGPT2API_ROOT", root)
	t.Setenv("CHATGPT2API_AUTH_KEY", "admin-secret")
	unsetLinuxDoEnv(t)
	...
	got, err := store.Update(map[string]any{
		...
		"email_smtp_auth_code":  "qq-auth-code",
		"email_smtp_from_email": "sender@qq.com",
		"email_smtp_from_name":  "chatgpt2api",
	})
	if err != nil {
		t.Fatalf("Update() error = %v", err)
	}

	assertConfigValue(t, got, "email_smtp_enabled", true)
	assertConfigValue(t, got, "email_smtp_host", "smtp.qq.com")
	assertConfigValue(t, got, "email_smtp_port", 465)
	assertConfigValue(t, got, "email_smtp_use_ssl", true)
	assertConfigValue(t, got, "email_smtp_username", "sender@qq.com")
	assertConfigValue(t, got, "email_smtp_from_email", "sender@qq.com")
	assertConfigValue(t, got, "email_smtp_from_name", "chatgpt2api")
	assertConfigValue(t, got, "email_smtp_auth_code_configured", true)
	if _, ok := got["email_smtp_auth_code"]; ok {
		t.Fatalf("Get() leaked email_smtp_auth_code: %#v", got)
	}
	...
	envText := string(envData)
	for _, want := range []string{
		"CHATGPT2API_EMAIL_SMTP_ENABLED=true",
		"CHATGPT2API_EMAIL_SMTP_HOST=smtp.qq.com",
		"CHATGPT2API_EMAIL_SMTP_PORT=465",
		"CHATGPT2API_EMAIL_SMTP_USE_SSL=true",
		"CHATGPT2API_EMAIL_SMTP_USERNAME=sender@qq.com",
		"CHATGPT2API_EMAIL_SMTP_AUTH_CODE=qq-auth-code",
		"CHATGPT2API_EMAIL_SMTP_FROM_EMAIL=sender@qq.com",
		"CHATGPT2API_EMAIL_SMTP_FROM_NAME=chatgpt2api",
	} {
		if !strings.Contains(envText, want) {
			t.Fatalf(".env missing %q in:\n%s", want, envText)
		}
	...
	if err != nil {
		t.Fatalf("Update() with blank auth code error = %v", err)
	}
	assertConfigValue(t, got, "email_smtp_auth_code_configured", true)
	if store.EmailSMTP().AuthCode != "qq-auth-code" {
		t.Fatalf("blank auth code update should preserve existing auth code")
	}
}

func TestStoreUpdateRejectsIncompleteEmailSMTPSettings(t *testing.T) {
	root := t.TempDir()
	t.Setenv("CHATGPT2API_ROOT", root)
	t.Setenv("CHATGPT2API_AUTH_KEY", "admin-secret")
	unsetLinuxDoEnv(t)
	...
	if err == nil || !strings.Contains(strings.ToLower(err.Error()), "auth code") {
		t.Fatalf("Update() error = %v, want missing auth code", err)
	}
}
...
func TestCleanupOldImagesThrottledByInterval(t *testing.T) {
	root := t.TempDir()
	t.Setenv("CHATGPT2API_ROOT", root)
	t.Setenv("CHATGPT2API_AUTH_KEY", "admin-secret")
	unsetLinuxDoEnv(t)
	...
	oldImagePath := filepath.Join(store.ImagesDir(), "2026", "01", "01", "old.png")
	if err := os.MkdirAll(filepath.Dir(oldImagePath), 0o755); err != nil {
		t.Fatalf("MkdirAll() error = %v", err)
	}
	if err := os.WriteFile(oldImagePath, []byte("image"), 0o644); err != nil {
		t.Fatalf("WriteFile() error = %v", err)
	}
	oldTime := time.Now().Add(-45 * 24 * time.Hour)
	if err := os.Chtimes(oldImagePath, oldTime, oldTime); err != nil {
		t.Fatalf("Chtimes() error = %v", err)
	}

	store.mu.Lock()
	store.lastImageCleanupAt = time.Now()
	store.mu.Unlock()

	if removed := store.CleanupOldImages(); removed != 0 {
		t.Fatalf("CleanupOldImages() first removed = %d, want 0 due to throttle", removed)
	}
	if _, err := os.Stat(oldImagePath); err != nil {
		t.Fatalf("expected old image to remain during throttle, stat error = %v", err)
	}

	store.mu.Lock()
	store.lastImageCleanupAt = time.Time{}
	store.mu.Unlock()

	if removed := store.CleanupOldImages(); removed < 1 {
		t.Fatalf("CleanupOldImages() second removed = %d, want >= 1", removed)
	}
	if _, err := os.Stat(oldImagePath); !os.IsNotExist(err) {
		t.Fatalf("expected old image removed after throttle window reset, stat error = %v", err)
	}
}
...
func unsetEmailSMTPEnv(t *testing.T) {
	t.Helper()
	for _, key := range []string{
		"CHATGPT2API_EMAIL_SMTP_ENABLED",
		"CHATGPT2API_EMAIL_SMTP_HOST",
		"CHATGPT2API_EMAIL_SMTP_PORT",
		"CHATGPT2API_EMAIL_SMTP_USE_SSL",
		"CHATGPT2API_EMAIL_SMTP_USERNAME",
		"CHATGPT2API_EMAIL_SMTP_AUTH_CODE",
		"CHATGPT2API_EMAIL_SMTP_FROM_EMAIL",
		"CHATGPT2API_EMAIL_SMTP_FROM_NAME",
	} {
		unsetEnv(t, key)
	}
}
