...
func TestRequiresPaidImageSize(t *testing.T) {
	tests := []struct {
		...
		{name: "1080p widescreen", size: "1920x1080", want: false},
...
func TestNormalizeImageGenerationSizeTiers(t *testing.T) {
	tests := []struct {
		...
		{size: "", want: ""},
		{size: "16:9", want: "16:9"},
		{size: "1080p", want: "1024x1024"},
		{size: " 2K ", want: "2048x2048"},
		{size: "4k", want: "4096x4096"},
...
func TestConversationRequestNormalizesResolutionTierSize(t *testing.T) {
	...
	if request.RequirePaidAccount {
		t.Fatal("Normalized() RequirePaidAccount = true, want false for legacy image format")
	}
}
...
func TestResponseImageGenerationToolAcceptsTypedToolSlice(t *testing.T) {
	...
	if request.RequirePaidAccount {
		t.Fatal("RequirePaidAccount = true, want false for legacy image format")
	}
}
