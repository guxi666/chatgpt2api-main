...
export const IMAGE_QUALITY_OPTIONS = [
  { value: "low", label: "Low", description: "低画质，速度更快，适合草稿测试" },
  { value: "medium", label: "Medium", description: "均衡质量与速度，适合日常生成" },
  { value: "high", label: "High", description: "高画质，耗时更长，适合最终出图" },
] as const;
...
export function calculateImageSize(resolution: Exclude<ImageResolution, "auto">, ratio: string) {
  ...
  const longEdge = resolution === "1k" ? 1024 : resolution === "2k" ? 2048 : 4096;
  const { width: ratioWidth, height: ratioHeight } = parsed;
  if (ratioWidth === ratioHeight) {
    return normalizeImageSize(`${longEdge}x${longEdge}`);
  }

  const width =
    ratioWidth > ratioHeight
      ? longEdge
      : roundToMultiple((longEdge * ratioWidth) / ratioHeight, SIZE_MULTIPLE);
  const height =
    ratioWidth > ratioHeight
      ? roundToMultiple((longEdge * ratioHeight) / ratioWidth, SIZE_MULTIPLE)
      : longEdge;
